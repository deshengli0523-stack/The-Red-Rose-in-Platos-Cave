"""Client-scope deletion lifecycle and durable rebuild-control schema."""
...
_STATEMENTS = (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS lifecycle_plan_objects (
    ...
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_plan_objects_no_update
    BEFORE UPDATE ON lifecycle_plan_objects BEGIN
        SELECT RAISE(ABORT, 'lifecycle plan object is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_plan_objects_no_delete
    BEFORE DELETE ON lifecycle_plan_objects BEGIN
        SELECT RAISE(ABORT, 'lifecycle plan object is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_plan_objects_no_replace
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS lifecycle_approval_attestations (
        operation_id TEXT PRIMARY KEY
    ...
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_approval_attestations_no_update
    BEFORE UPDATE ON lifecycle_approval_attestations BEGIN
        SELECT RAISE(ABORT, 'lifecycle approval attestation is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_approval_attestations_no_delete
    BEFORE DELETE ON lifecycle_approval_attestations BEGIN
        SELECT RAISE(ABORT, 'lifecycle approval attestation is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS lifecycle_approval_attestations_no_replace
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS case_publication_proofs (
        ...
        approval_descriptor_base_version INTEGER NOT NULL CHECK(
            approval_descriptor_base_version >= 0
        ),
        approval_applied_commit_version INTEGER NOT NULL CHECK(
            approval_applied_commit_version > 0
        ),
        ...
        global_publication_operation_id TEXT NOT NULL UNIQUE CHECK(
            length(global_publication_operation_id) > 0
        ),
        publication_closure_sha256 TEXT NOT NULL CHECK(
            length(publication_closure_sha256) = 64
            AND publication_closure_sha256 NOT GLOB '*[^0-9a-f]*'
        ),
        manifest_id TEXT NOT NULL CHECK(length(manifest_id) > 0),
        published_global_version INTEGER NOT NULL CHECK(
            published_global_version > 0
        ),
        attestor_id TEXT NOT NULL CHECK(
            length(attestor_id) BETWEEN 1 AND 64
            AND attestor_id NOT GLOB '*[^a-z0-9_-]*'
        ),
    ...
    """,
    """
    CREATE TRIGGER IF NOT EXISTS case_publication_proofs_no_update
    BEFORE UPDATE ON case_publication_proofs BEGIN
        SELECT RAISE(ABORT, 'case publication proof is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS case_publication_proofs_no_delete
    BEFORE DELETE ON case_publication_proofs BEGIN
        SELECT RAISE(ABORT, 'case publication proof is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS case_publication_proofs_no_replace
    BEFORE INSERT ON case_publication_proofs
    WHEN EXISTS(
        SELECT 1 FROM case_publication_proofs
         WHERE event_id = NEW.event_id
            OR approval_operation_id = NEW.approval_operation_id
            OR global_publication_operation_id
                = NEW.global_publication_operation_id
            OR proof_sha256 = NEW.proof_sha256
    )
    BEGIN
        SELECT RAISE(ABORT, 'case publication proof is immutable');
    END
    """,
    """
    CREATE TRIGGER IF NOT EXISTS outbox_events_publication_proof_required
    BEFORE UPDATE OF state, published_global_version ON outbox_events
    WHEN NEW.state = 'PUBLISHED'
         AND NOT EXISTS(
             SELECT 1 FROM case_publication_proofs AS proof
              WHERE proof.event_id = NEW.event_id
                AND proof.published_global_version
                    = NEW.published_global_version
         )
    BEGIN
        SELECT RAISE(ABORT, 'published outbox event requires signed proof');
    END
    """,
    """
    CREATE TABLE IF NOT EXISTS deletion_requests (
        ...
        target_type TEXT NOT NULL CHECK(
            target_type IN ('client', 'session', 'case', 'passage', 'claim')
        ),
...
def upgrade(connection: sqlite3.Connection) -> None:
    """Install hash-only client lifecycle control tables in the runner transaction."""
