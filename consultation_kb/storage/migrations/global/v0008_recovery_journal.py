"""Body-free production recovery journal and required-action queue."""
...
_STATEMENTS = (
    ...
    SELECT operation.operation_id, authority.catalog_version,
           authority.authorization_epoch, authority.tombstone_epoch,
           'LEGACY_UNTRUSTED', operation.created_at
      FROM publication_operations AS operation
      JOIN knowledge_catalog_state AS authority ON authority.singleton = 1
    """,
    """
    ...
    AFTER INSERT ON publication_operations BEGIN
        ...
        SELECT NEW.operation_id, authority.catalog_version,
               authority.authorization_epoch, authority.tombstone_epoch,
               'LIVE', NEW.created_at
          FROM knowledge_catalog_state AS authority
